...
<?php if ( isset( $this->parent->args['intro_text'] ) ) { ?>
    <div id="reduk-intro-text"><?php echo wp_kses_post( $this->parent->args['intro_text'] ); ?></div>
<?php } ?>

<?php $this->get_template( 'menu_container.tpl.php' ); ?>

<div class="reduk-main">
    <!-- Stickybar -->
    <?php $this->get_template( 'header_stickybar.tpl.php' ); ?>
    <div id="reduk_ajax_overlay">&nbsp;</div>
    <?php
        ...
        echo '<div id="' . $k . '_section_group' . '" class="reduk-group-tab' . esc_attr( $section['class'] ) . '" data-rel="' . $k . '">';
        //echo '<div id="' . $k . '_nav-bar' . '"';
        /*
    if ( !empty( $section['tab'] ) ) {

        echo '<div id="' . $k . '_section_tabs' . '" class="reduk-section-tabs">';
        ...
        echo '</ul>';
        foreach ($section['tab'] as $subkey => $subsection) {
            echo '<div id="' . $k .'sub-'.$subkey. '_section_group' . '" class="reduk-group-tab" style="display:block;">';
        ...
        */
        ...
        if ( $display ) {
            do_action( "reduk/page/{$this->parent->args['opt_name']}/section/before", $section );
            $this->output_section( $k );
            do_action( "reduk/page/{$this->parent->args['opt_name']}/section/after", $section );
        }
        //}
    ?></div><?php
    //print '</div>';
    ...
    /**
     * action 'reduk/page-after-sections-{opt_name}'
     *
     * @deprecated
     *
     * @param object $this RedukFramework
     */
    do_action( "reduk/page-after-sections-{$this->parent->args['opt_name']}", $this ); // REMOVE LATER

    /**
     * action 'reduk/page/{opt_name}/sections/after'
     *
     * @param object $this RedukFramework
     */
    do_action( "reduk/page/{$this->parent->args['opt_name']}/sections/after", $this );
?>
<div class="clear"></div>
<!-- Footer Block -->
