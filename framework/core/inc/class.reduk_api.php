    ...
    if ( ! class_exists( 'Reduk' ) ) {
        ...
        class Reduk {
            ...
            public static function load() {
                add_action( 'after_setup_theme', array( 'Reduk', 'createReduk' ) );
                add_action( 'init', array( 'Reduk', 'createReduk' ) );
                add_action( 'switch_theme', array( 'Reduk', 'createReduk' ) );
            }

            public static function init( $opt_name = "" ) {
                if ( ! empty( $opt_name ) ) {
                    self::loadReduk( $opt_name );
                    remove_action( 'setup_theme', array( 'Reduk', 'createReduk' ) );
                }
            }

            public static function loadExtensions( $RedukFramework ) {
                if ( $instanceExtensions = self::getExtensions( $RedukFramework->args['opt_name'], "" ) ) {
                    foreach ( $instanceExtensions as $name => $extension ) {
                        if ( ! class_exists( $extension['class'] ) ) {
                            // In case you wanted override your override, hah.
                            $extension['path'] = apply_filters( 'reduk/extension/' . $RedukFramework->args['opt_name'] . '/' . $name, $extension['path'] );
                            if ( file_exists( $extension['path'] ) ) {
                                require_once $extension['path'];
                            }
                        }
                        if ( ! isset( $RedukFramework->extensions[ $name ] ) ) {
                            if ( class_exists( $extension['class'] ) ) {
                                $RedukFramework->extensions[ $name ] = new $extension['class']( $RedukFramework );
                            } else {
            ...
            public static function extensionPath( $extension, $folder = true ) {
                if ( ! isset( Reduk::$extensions[ $extension ] ) ) {
                    return;
                }
                $path = end( Reduk::$extensions[ $extension ] );
                if ( ! $folder ) {
                    return $path;
                }
            ...
            public static function loadReduk( $opt_name = "" ) {

                if ( empty( $opt_name ) ) {
                    return;
                }

                $check = RedukFrameworkInstances::get_instance( $opt_name );
                ...
                if ( ! class_exists( 'RedukFramework' ) ) {
                    echo '<div id="message" class="error"><p>Reduk Framework is <strong>not installed</strong>. Please install it.</p></div>';

                    return;
                }
                if ( isset( self::$uses_extensions[ $opt_name ] ) && ! empty( self::$uses_extensions[ $opt_name ] ) ) {
                    add_action( "reduk/extensions/{$opt_name}/before", array( 'Reduk', 'loadExtensions' ), 0 );
                }

                $reduk                   = new RedukFramework( $sections, $args );
                $reduk->apiHasRun        = 1;
                self::$init[ $opt_name ] = 1;
                if ( isset( $reduk->args['opt_name'] ) && $reduk->args['opt_name'] != $opt_name ) {
                    self::$init[ $reduk->args['opt_name'] ] = 1;
                }

            }

            public static function createReduk() {
            ...
            public static function setSections( $opt_name = '', $sections = array() ) {
                ...
                if ( ! empty( $sections ) ) {
                    foreach ( $sections as $section ) {
                        Reduk::setSection( $opt_name, $section );
                    }
                }
            }
            ...
            public static function setOption ($opt_name = "", $key = "", $option = "") {
                ...
                if (!empty($opt_name) && !empty($key)) {
                    ...
                    return update_option($opt_name, $reduk);
                } else {
                    return false;
                }
            }
            ...
            public static function checkExtensionClassFile( $opt_name, $name = "", $class_file = "", $instance = "" ) {
                if ( file_exists( $class_file ) ) {
                    ...
                    $version                   = Reduk_Helpers::get_template_version( $class_file );
            ...
            public static function getAllExtensions() {
                ...
                foreach ( $reduk as $instance ) {
                    if ( ! empty( self::$uses_extensions[ $instance['args']['opt_name'] ] ) ) {
                        continue;
                    }
                    if ( ! empty( $instance['extensions'] ) ) {

                        Reduk::getInstanceExtensions( $instance['args']['opt_name'], $instance );
                    }
                }
            }
            ...
            public static function getInstanceExtensions( $opt_name, $instance = array() ) {
                ...
                if ( empty( $instance ) ) {
                    $instance = RedukFrameworkInstances::get_instance( $opt_name );
                }
                ...
                foreach ( $instance->extensions as $name => $extension ) {
                    if ( $name == "widget_areas" ) {
                        $new = new Reduk_Widget_Areas( $instance );
                    }
                    if ( isset( self::$uses_extensions[ $opt_name ][ $name ] ) ) {
                        continue;
                    }
                    if ( isset( $extension->extension_dir ) ) {
                        Reduk::setExtensions( $opt_name, str_replace( $name, '', $extension->extension_dir ) );

                    } else if ( isset( $extension->_extension_dir ) ) {
                        Reduk::setExtensions( $opt_name, str_replace( $name, '', $extension->_extension_dir ) );
                    }
                }
            }
            ...
            public static function getExtensions( $opt_name = "", $key = "" ) {
                ...
                if ( empty( $opt_name ) ) {
                    ...
                    if ( empty( self::$uses_extensions[ $opt_name ] ) ) {
                        Reduk::getInstanceExtensions( $opt_name );
                    }
                    ...
                    foreach ( self::$uses_extensions[ $opt_name ] as $extension ) {
                        ...
                        $extension_class                  = 'RedukFramework_Extension_' . $name;
