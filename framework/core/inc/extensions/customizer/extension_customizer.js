...
(function( $ ) {  //This functions first parameter is named $
    ...
    reduk.customizer = reduk.customizer || {};

    $( document ).ready(
        function() {
            reduk.customizer.init();
        }
    );
    reduk.customizer.init = function() {
        $( 'body' ).addClass( reduk_customizer.body_class );
        $( '.accordion-section.reduk-section, .accordion-section.reduk-panel, .accordion-section-title' ).click(
            function() {
                $.reduk.initFields();
            }
        );
    };
    reduk.customizer.save = function( $obj, $container ) {
        var $parent = $obj.hasClass( 'reduk-field' ) ? $obj : $obj.parents( '.reduk-field-container:first' );
        reduk.customizer.inputSave( $parent );
    };
    reduk.customizer.inputSave = function( $parent ) {

        if ( !$parent.hasClass( 'reduk-field-container' ) ) {
            $parent = $parent.parents( '[class^="reduk-field-container"]' );
        }

        var $id = $parent.parent().find( '.reduk-customizer-input' ).data( 'id' );

        if ( !$id ) {
            $parent = $parent.parents( '.reduk-container-repeater:first' );
            var $id = $parent.parent().find( '.reduk-customizer-input' ).data( 'id' );
        }
        ...
        var $key = $parent.parent().find( '.reduk-customizer-input' ).data( 'key' );
        if ( $nData[$key] ) {
            $nData = $nData[$key];
        }
