...
(function( $ ) {
    ...
    reduk.field_objects.slider = reduk.field_objects.slider || {};
    ...
    reduk.field_objects.slider.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".reduk-group-tab:visible" ).find( '.reduk-container-slider:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( 'div.reduk-slider-container' ).each(
                    function() {
                        ...
                        if ( displayValue == DISPLAY_TEXT ) {
                            defClassOne = el.find( '.reduk-slider-input-one-' + mainID );
                            defClassTwo = el.find( '.reduk-slider-input-two-' + mainID );

                            inputOne = defClassOne;
                            inputTwo = defClassTwo;
                        } else if ( displayValue == DISPLAY_SELECT ) {
                            defClassOne = el.find( '.reduk-slider-select-one-' + mainID );
                            defClassTwo = el.find( '.reduk-slider-select-two-' + mainID );

                            reduk.field_objects.slider.loadSelect( defClassOne, minVal, maxVal, resVal, stepVal );

                            if ( handles === 2 ) {
                                reduk.field_objects.slider.loadSelect( defClassTwo, minVal, maxVal, resVal, stepVal );
                            }

                        } else if ( displayValue == DISPLAY_LABEL ) {
                            defClassOne = el.find( '#reduk-slider-label-one-' + mainID );
                            defClassTwo = el.find( '#reduk-slider-label-two-' + mainID );
                        } else if ( displayValue == DISPLAY_NONE ) {
                            defClassOne = el.find( '.reduk-slider-value-one-' + mainID );
                            defClassTwo = el.find( '.reduk-slider-value-two-' + mainID );
                        }
                                ...
                                slide: function() {
                                    if ( displayValue == DISPLAY_LABEL ) {
                                        if ( handles === 2 ) {
                                            ...
                                            el.find( 'input.reduk-slider-value-one-' + mainID ).attr(
                                                'value', inpSliderVal[0]
                                            );
                                            el.find( 'input.reduk-slider-value-two-' + mainID ).attr(
                                    ...
                                    if ( displayValue == DISPLAY_SELECT ) {
                                        if ( handles === 2 ) {
                                            el.find( '.reduk-slider-select-one' ).select2( 'val', slider.val()[0] );
                                            el.find( '.reduk-slider-select-two' ).select2( 'val', slider.val()[1] );
                                        } else {
                                            el.find( '.reduk-slider-select-one' ).select2( 'val', slider.val() );
                                        }
                                    }

                                    reduk_change( $( this ).parents( '.reduk-field-container:first' ).find( 'input' ) );
                                }
                            }
                        );
                ...
                el.find( 'select.reduk-slider-select-one, select.reduk-slider-select-two' ).select2( default_params );
    ...
    reduk.field_objects.slider.isFloat = function( mixed_var ) {
        return +mixed_var === mixed_var && (!(isFinite( mixed_var ))) || Boolean( (mixed_var % 1) );
    };

    // Return number of integers after the decimal point.
    reduk.field_objects.slider.decimalCount = function( res ) {
        var q = res.toString().split( '.' );
        return q[1].length;
    };

    reduk.field_objects.slider.loadSelect = function( myClass, min, max, res, step ) {
        ...
        for ( var i = min; i <= max; i = i + res ) {
            ...
            var n = i;
            if ( reduk.field_objects.slider.isFloat( res ) ) {
                var decCount = reduk.field_objects.slider.decimalCount( res );
