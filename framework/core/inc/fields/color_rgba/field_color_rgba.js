(function($){
    ...
    reduk.field_objects.color_rgba          = reduk.field_objects.color_rgba || {};
    reduk.field_objects.color_rgba.fieldID  = '';

    reduk.field_objects.color_rgba.hexToRGBA = function( hex, alpha ) {
        var result;
    ...
    reduk.field_objects.color_rgba.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".reduk-group-tab:visible" ).find( '.reduk-container-color_rgba:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                reduk.field_objects.color_rgba.modInit(el);
                reduk.field_objects.color_rgba.initColorPicker(el);
            });
    };

    reduk.field_objects.color_rgba.modInit = function(el) {

        reduk.field_objects.color_rgba.fieldID    = el.find('.reduk-color_rgba-container').data('id');

    };

    // Initialize colour picker
    reduk.field_objects.color_rgba.initColorPicker = function(el){

        // Get field ID
        var field_id    = reduk.field_objects.color_rgba.fieldID;

        // Get the color scheme container
        var colorpickerInput = el.find('.reduk-color-rgba');
        ...
        var container   = el.find('.reduk-color-rgba-container');
        ...
        colorpickerInput.spectrum({
            ...
            localStorageKey:        'reduk.color-rgba.' + field_id,
            palette:                palette,
            inputText:              inputText,
            ...
            change: function(color) {
                ...
                if (colorVal != 'transparent') {
                    rgbaVal     = reduk.field_objects.color_rgba.hexToRGBA(colorVal, alphaVal);
                } else {
                    rgbaVal     = 'transparent';
                }
                ...
                reduk_change(el.find('.reduk-color-rgba-container'));
            }
        });
    };
})(jQuery);