...
// Don't duplicate me!
if( !class_exists( 'RedukFramework_color_rgba' ) ) {

    /**
     * Main RedukFramework_color_rgba class
     *
     * @since       1.0.0
     */
    class RedukFramework_color_rgba {
       ...
       */
        ...
        public function render() {
            ...
            echo '<div 
                      class="reduk-color-rgba-container ' . $this->field['class'] . '" 
                      data-id="'                        . $field_id . '"
            ...
            if ('' == $this->value['color'] || 'transparent' == $this->value['color']) {
                ...
                $color = Reduk_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }            

            if ($this->value['rgba'] == '' && $this->value['color'] != '' ){
                $this->value['rgba'] = Reduk_Helpers::hex2rgba($this->value['color'], $this->value['alpha']);
            }
            
            echo '<input
                        name="' . $this->field['name'] . $this->field['name_suffix'] . '[color]"
                        id="' . $field_id . '-color"
                        class="reduk-color-rgba"
                        type="text"
        ...
        public function enqueue() {
            ...
            // Field dependent JS
            if (!wp_script_is ( 'reduk-field-color-rgba-js' )) {
                wp_enqueue_script(
                    'reduk-field-color-rgba-js', 
                    RedukFramework::$_url . 'inc/fields/color_rgba/field_color_rgba' . Reduk_Functions::isMin() . '.js',
                    array('jquery', 'reduk-spectrum-js'), 
                    time(), 
                    true
                );
            }
            ...
            // Spectrum CSS
            if (!wp_style_is ( 'reduk-spectrum-css' )) {
                wp_enqueue_style('reduk-spectrum-css');
            }
            
            if ($this->parent->args['dev_mode']) {
                if (!wp_style_is ( 'reduk-field-color-rgba-css' )) {
                    wp_enqueue_style(
                        'reduk-field-color-rgba-css',
                        RedukFramework::$_url . 'inc/fields/color_rgba/field_color_rgba.css',
                        array(),
                        time(),
                        'all'
                    );
            ...
            if (is_array($this->value)) {
                ...
                foreach($this->value as $id => $val) {
                    ...
                    if ($id == 'alpha') {
                        ...
                        $rgba = !empty($val) ? $val : '';
                        $rgba = Reduk_Helpers::hex2rgba($color, $alpha);
                    }
                }
        ...
        public function output() {
            if (!empty($this->value)) {
                ...
                if ( ! empty( $this->field['output'] ) && is_array( $this->field['output'] ) ) {
                    if (!empty($color_val)) {
                        $css = Reduk_Functions::parseCSS( $this->field['output'], $style, $color_val );
                ...
                if ( ! empty( $this->field['compiler'] ) && is_array( $this->field['compiler'] ) ) {
                    if (!empty($color_val)) {
                        $css = Reduk_Functions::parseCSS( $this->field['compiler'], $style, $color_val );
