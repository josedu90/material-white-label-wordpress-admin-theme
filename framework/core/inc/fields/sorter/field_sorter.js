...
(function( $ ) {
    ...
    reduk.field_objects.sorter = reduk.field_objects.sorter || {};

    var scroll = '';

    $( document ).ready(
        function() {
            //reduk.field_objects.sorter.init();
        }
    );

    reduk.field_objects.sorter.init = function( selector ) {

        if ( !selector ) {
            selector = $( document ).find( ".reduk-group-tab:visible" ).find( '.reduk-container-sorter:visible' );
        }
            ...
            function() {
                ...
                el.find( '.reduk-sorter' ).each(
                    function() {
                        ...
                        el.find( '#' + id ).find( 'ul' ).sortable(
                            {
                                ...
                                out: function( event, ui ) {
                                    ...
                                    reduk.field_objects.sorter.scrolling( $( this ).parents( '.reduk-field-container:first' ) );
                                ...
                                stop: function( event, ui ) {
                                    var sorter = reduk.sorter[$( this ).attr( 'data-id' )];
                                ...
                                update: function( event, ui ) {
                                    ...
                                    $( this ).find( '.position' ).each(
                                        function() {
                                            ...
                                            $( this ).prop(
                                                "name",
                                                reduk.args.opt_name + '[' + optionID + '][' + parentID + '][' + listID + ']'
                                            );
                                        }
                                    );
                                }
                        ...
                        el.find( ".reduk-sorter" ).disableSelection();
                    }
                );
            }
        );
    ...
    reduk.field_objects.sorter.scrolling = function( selector ) {
        ...
        if ( scroll == 'up' ) {
            ...
            setTimeout( reduk.field_objects.sorter.scrolling, 50 );
        } else if ( scroll == 'down' ) {
            scrollable.scrollTop( scrollable.scrollTop() + 20 );
            setTimeout( reduk.field_objects.sorter.scrolling, 50 );
        }
