...
// Don't duplicate me!
    if ( ! class_exists( 'RedukFramework_section' ) ) {

        /**
         * Main RedukFramework_heading class
         *
         * @since       1.0.0
         */
        class RedukFramework_section {
            ...
            public function render() {
                ...
                echo '<div id="section-' . esc_attr($this->field['id']) . '" class="reduk-section-field reduk-field ' . esc_attr($this->field['style']) . ' ' . esc_attr($this->field['class']) . ' ">';
                ...
                if ( ! empty( $this->field['subtitle'] ) ) {
                    echo '<div class="reduk-section-desc">' . esc_html($this->field['subtitle']) . '</div>';
                }
                ...
                <script type="text/javascript">
                    jQuery( document ).ready(
                        function() {
                            ...
                            var group = jQuery( '#<?php echo esc_js($this->field['id']); ?>-marker' ).parents( '.reduk-group-tab:first' );
                            if ( !group.hasClass( 'sectionsChecked' ) ) {
                                group.addClass( 'sectionsChecked' );
                                var test = group.find( '.reduk-section-indent-start h3' );
                                jQuery.each(
            ...
            public function enqueue() {
                if ( $this->parent->args['dev_mode'] ) {
                    wp_enqueue_style(
                        'reduk-field-section-css',
                        RedukFramework::$_url . 'inc/fields/section/field_section.css',
                        array(),
                        time(),
                        'all'
                    );
