/*global reduk_change, wp, reduk, libFilter */
...
(function($){
    ...
    reduk.field_objects.media   = reduk.field_objects.media || {};
    ...
    reduk.field_objects.media.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".reduk-group-tab:visible" ).find( '.reduk-container-media:visible' );
        }
        $( selector ).each(
            function() {
                var el = $( this );
                ...
                el.find( '.remove-image, .remove-file' ).unbind( 'click' ).on(
                    'click', function() {
                        reduk.field_objects.media.removeFile( $( this ).parents( 'fieldset.reduk-field:first' ) );
                    }
                );
                // Upload media button
                el.find( '.media_upload_button' ).unbind().on(
                    'click', function( event ) {
                        reduk.field_objects.media.addFile( event, $( this ).parents( 'fieldset.reduk-field:first' ) );
                    }
                );
            }
        );
    ...
    reduk.field_objects.media.addFile = function( event, selector ) {
        ...
        frame.on(
            'select', function() {
                ...
                if ( typeof reduk.field_objects.media === 'undefined' || typeof reduk.field_objects.media === undefined ) {
                    reduk.field_objects.media = {};
                }
                ...
                selector.find( '.reduk-background-properties' ).slideDown();
            }
        );
    ...
    reduk.field_objects.media.removeFile = function( selector ) {
        ...
        reduk_change( $( selector ).find( '.upload-id' ) );
        selector.find( '.reduk-background-properties' ).hide();
